...
impl IndexedTimsTOFData {
    ...
    pub fn from_timstof_data(data: TimsTOFData) -> Self {
        ...
        // 2. 并行快速排序，比较函数直接使用 f32 值
        use rayon::prelude::*;
        order.par_sort_unstable_by(|&a, &b| {
            data.mz_values[a]
                .partial_cmp(&data.mz_values[b])
                .unwrap()
        });

        // 3. 按排好序的下标重新收集各列
        Self {
            rt_values_min: order.par_iter().map(|&i| data.rt_values_min[i]).collect(),
            mobility_values: order.par_iter().map(|&i| data.mobility_values[i]).collect(),
            mz_values: order.par_iter().map(|&i| data.mz_values[i]).collect(),
            intensity_values: order.par_iter().map(|&i| data.intensity_values[i]).collect(),
            frame_indices: order.par_iter().map(|&i| data.frame_indices[i]).collect(),
            scan_indices: order.par_iter().map(|&i| data.scan_indices[i]).collect(),
        }
    }
