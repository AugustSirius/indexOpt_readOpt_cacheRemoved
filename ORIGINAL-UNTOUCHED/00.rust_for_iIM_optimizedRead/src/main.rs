...
use utils::{
    read_timstof_data, build_indexed_data,
    library_records_to_dataframe, get_unique_precursor_ids, 
    process_library_fast, create_rt_im_dicts, 
    LibCols, PrecursorLibData, prepare_precursor_lib_data,
    extract_unique_rt_im_values, save_unique_values_to_files
};
use processing::{
    FastChunkFinder,
    process_single_precursor_compressed
};

use rayon::prelude::*;
use std::{error::Error, path::Path, time::Instant, fs::File};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (ms1_indexed, ms2_indexed_pairs, all_im_values) = (ms1_indexed, ms2_indexed_pairs, all_im_values);
...
fn save_results_as_separate_files(
    ...
    // Save IM matrix
    let npy_file_path = Path::new(output_dir).join("all_im_matrix.npy");
    println!("Saving IM matrix to: {:?}", npy_file_path);
    write_npy(npy_file_path, &all_im_matrix)?;
    ...
    let id_file_path = Path::new(output_dir).join("precursor_ids_IM.txt");
